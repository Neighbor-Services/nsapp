...
file_path = 'e:/ns/frontend/nsapp/lib/features/messages/data/repository/messages_repository_impl.dart'
...
content = re.sub(r'if\s*\(results\s*!=\s*null\)\s*\{([\s\S]*?)\}\s*return\s+Left\(\s*Failure\(message:\s*"An error occurred and no cached data found"\),\s*\);', r'\1', content)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print('Updated message repository null checks')
