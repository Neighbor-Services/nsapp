        ...
        listener: (context, state) {
          ...
          if (state is FailureAddToFavoriteState) {
            customAlert(
              context,
              AlertType.error,
              'Failed to add favorite: ${state.message}',
            );
          }
          if (state is FailureRemoveFromFavoriteState) {
            customAlert(
              context,
              AlertType.error,
              'Failed to remove favorite: ${state.message}',
            );
          }
        },
        builder: (context, state) {
          // Extract favorites list reactively for favorite checks
          final favorites = context.read<SeekerBloc>().myFavorites; // Resolve the canonical provider list from state first, fallback to cached
          final List<Profile> stateProviders =
              state is SuccessSearchProviderState ? state.providers : providers;
                        ...
                        child: Builder(
                          builder: (context) {
                            ...
                            if (state is LoadingSeekerState &&
                                stateProviders.isEmpty) {
                              return const LoadingWidget();
                            }
                            final List<Profile> displayList = isSearching
                                ? searchedProviders
                                : stateProviders;
